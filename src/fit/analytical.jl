...
(Ridge) with an analytical (or analytical-based) method and in the case where
the number of rows `n` dominates `p`.
...
Hessian X'X.
* iterative (conjugate gradient): O(κnp) - with κ the number of CG steps (κ ≤
p).
"""
function _fit(glr::GLR{L2Loss,<:L2R}, solver::Analytical{:tall}, X, y)
	# full solve
	if !solver.iterative
...
function _fit(glr::GLR{L2Loss,<:L2R}, solver::Analytical{:fat}, X, y)
	# full solve
	if !solver.iterative
		λ  = getscale(glr.penalty)

		# Ridge case -- form the Hat Matrix then solve
		XXt = form_XXt(X, glr.fit_intercept, λ)
		H = XXt + λ * I
		b = XXt * y
		glr.fit_intercept && (b = vcat(b, sum(y)))
		return cholesky!(H) \ b
